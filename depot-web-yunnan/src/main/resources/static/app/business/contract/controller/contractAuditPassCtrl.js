angular.module('app.business').controller("contractAuditPassCtrl", function($scope, $http, $state, $stateParams,$uibModal, contractService, APP_CONFIG) {
	...
	$scope.menu = $stateParams.menu;
     ...
     $scope.loadData = function() {
    	 contractService.getPassPageInfo($scope.pageInfo.pageNum, $scope.pageInfo.pageSize, $scope.searchCondition).then(function(data){
             $scope.pageInfo = data;
         },function(data){
             console.log(data);
         });
    ...
    $scope.showView = function(id, processInstanceId) {
    	if($scope.menu!=null){
    		$state.go("app.business.management.contract-pass-view", {id:id, processInstanceId : processInstanceId});
    	}else{
