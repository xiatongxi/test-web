angular.module('app.business').controller("transferContractAuditPassSaveCtrl",
		function($scope, $rootScope, $http, $filter, $stateParams, $state,$uibModal,userService, 
		        contractService, transferContractService, enumService, APP_CONFIG) {
  ...
  //-----------------------------------------------暂停、终止、启用-------------------------------------
    $scope.pageInfo = {pageNum : 1, pageSize : 10};
     ...
     $scope.loadData = function() {
    	 transferContractService.getPassPageInfo($scope.pageInfo.pageNum, $scope.pageInfo.pageSize, null).then(function(data){
             $scope.pageInfo = data;
         },function(data){
             console.log(data);
         });
    ...
    $scope.application = function(id) {
        $http({
        	...
        	if (response.data.status == "error") {
        		...
        		// 重新加载数据
        		$state.go("app.business.transferContract-audit-pass");
        		$scope.loadData();
        		alert("启用成功！");
        	}
