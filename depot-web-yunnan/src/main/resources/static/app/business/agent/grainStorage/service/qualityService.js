...
angular.module('app.business').service("agentQualityService", function($http, $q, $rootScope,APP_CONFIG) {
	this.getPageInfo = function(pageNum, pageSize,type,checkResult,agentId,agentDepotId,storehouseId,warehouseId){
	...
	this.getNewPageInfo = function(pageNum, pageSize, checkType, checkResult, storehouseId, warehouseId, orgId, historyStatus){
		...
		$http({
			method : 'GET',
			url : APP_CONFIG.agentUrl + '/agentQuality/getNewList',
			params : {
				pageNum : pageNum,
				pageSize : pageSize,
                //type可以取的值：0:初检 1：验收 2：春秋普查 3：出库检验 4：日常检验 5：第三方检查
    ...
    this.loadDataById = function(id){
        ...
        $http({
            method: 'GET',
            url: APP_CONFIG.agentUrl + '/agentQuality/loadDataById',
            params: {
                id : id
            }
        }).then(function successCallback(response) {
    ...
    this.removeById = function(id){
        ...
        $http({
            method: 'POST',
            url: APP_CONFIG.agentUrl + '/agentQuality/remove',
            data: {
                id : id
            }
        }).then(function successCallback(response) {
    ...
    this.findByStoreWarehouse = function(houseId, warehouseId, type, checkResult) {
        ...
        $http({
            method: 'GET',
            url: APP_CONFIG.agentUrl + '/agentQuality/findByStoreWarehouse',
            params: {
                houseId : houseId,
                warehouseId:warehouseId,
                type:type,
    ...
    this.findByStoreWarehouseData = function(houseId,warehouseId,type,id) {
        ...
        $http({
            method: 'GET',
            url: APP_CONFIG.agentUrl + '/agentQuality/findByStoreWarehouseData',
            params: {
                houseId : houseId,
                warehouseId:warehouseId,
                type:type,
    ...
    this.getQualitycheckListByFoodbasicinfoCheck = function(houseId,warehouseId,foodbasicinfoId){
        ...
        $http({
            method : 'GET',
            url : APP_CONFIG.agentUrl + '/agentQuality/getQualitycheckListByFoodbasicinfoCheck',
            params : {
                houseId : houseId,
                warehouseId : warehouseId,
    ...
    this.findQualitycheckSonByQcSonId = function (qcSonId) {
        ...
        $http({
            method: 'GET',
            url: APP_CONFIG.agentUrl + '/agentQualitySon/findQualitycheckSonBySonId',
            params: {
                qcSonId:qcSonId,
            }
    ...
    this.removeSon = function(id) {
        ...
        $http({
            method: 'POST',
            url: APP_CONFIG.agentUrl + '/agentQualitySon/removeSon',
            data: {
                sonId : id
            }
        }).then(function successCallback(response) {
    ...
    //获取第三方检查的不宜存的可用数据
    this.getStoreQualityList = function() {
        var d = $q.defer();
        $http({
            method : 'GET',
            url : APP_CONFIG.agentUrl + '/agentQuality/getStoreQualityList',
        }).then(function successCallback(response) {
            // 请求成功执行代码
            d.resolve(response.data);
        }, function errorCallback(response) {
    ...
    this.getTrdStoreQualityList = function() {
    	...
    	$http({
    		method : 'GET',
    		url : APP_CONFIG.agentUrl + '/agentQuality/getTrdStoreQualityList',
    	}).then(function successCallback(response) {
    		// 请求成功执行代码
    		d.resolve(response.data);
    	}, function errorCallback(response) {
    ...
    this.setQualityCheckByHistoryStatus=function (profitLossSheet) {
        ...
        $http({
            method: 'POST',
            url: APP_CONFIG.agentUrl + '/agentQuality/setQualityCheckByHistoryStatus',
            data: {
    ...
    this.getDataByZJId = function(ZZid){
        var d = $q.defer();
        $http({
            method : 'GET',
            url : APP_CONFIG.agentUrl + '/agentQuality/getDataByZJId',
            params : {
                ZZid : ZZid
            }
        }).then(function successCallback(response) {
    ...
    this.getPageInfoByhouseId = function(pageNum, pageSize,type,houseIds,warehouseIds){
		...
		$http({
			method : 'GET',
			url : APP_CONFIG.agentUrl + '/agentQuality/getListByhouseId',
			params : {
				pageNum : pageNum,
				pageSize : pageSize,
                //type可以取的值：0:初检 1：验收 2：春秋普查 3：出库检验 4：日常检验 5：第三方检查
