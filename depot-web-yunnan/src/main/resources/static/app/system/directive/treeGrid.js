$(function () {
    ...
    var getChilds = function (node, source, field) {
    	var data = [];
    	var items = $.grep(source, function (item, index) {
    		return item.parentId == node[field];
    	});
    	...
    	$.each(items, function (index, item) {
    		data.push(item);
    		if(field != "enumid"){//为数据字典单独写的，原始代码没有if判断
    			var child = getChild(item, source, field);
    			$.each(child, function (i, n) {
    				data.push(n);
    			});
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        var indent = item.level == that.options.level ? '' : sprintf('<span style="margin-left: %spx;"></span>', (item.level - that.options.treeRootLevel) * 25);
        ...
        this.$body.find('> tr[data-index] > td > .tree-icon').off('click').on('click', function (e) {
            ...
            var $this = $(this),
                ...
                row = data[index];
            var icon = $(this);
            //原始代码就是if里的js，没有if和else判断，getChilds方法新加的
            if (icon.hasClass(that.options.expandIcon)) {//已展开的情况下点击获取所有子级
            	var child = getChild(data[index], data, that.options.treeId);
            }else{//未展开的情况下点击获取当前子级
            	var child = getChilds(data[index], data, that.options.treeId);
            }
